...
summary_stats <- healthcare_df %>%
  summarise(
    mean_age = mean(Age),
    median_bmi = median(BMI),
    sd_cost = sd(HealthcareCost),
    range_cost = max(HealthcareCost) - min(HealthcareCost)
  )
...
region_count <- healthcare_df %>%
  count(Region)

plot_region <- ggplot(region_count, aes(x = Region, y = n, fill = Region)) +
...
# - x = VisitType
# - y = Count
visit_count <- healthcare_df %>%
  count(VisitType)

plot_visit <- ggplot(visit_count, aes(x = VisitType, y = n, fill = VisitType)) +
  geom_bar(stat = "identity") +
...
plot_age_cost <- ggplot(healthcare_df, aes(x = HealthcareCost, y = Age)) +
  geom_point() +
...
plot_age_cost <- ggplot(healthcare_df, aes(x = Age, y = HealthcareCost, color = Smoker)) +
  geom_point() +
  labs(title = "Healthcare Cost by Age by Smoker Status", x = "Age", y = "Healthcare Cost")
print(plot_age_cost)
...
plot_cost_by_insurance <- ggplot(healthcare_df, aes(x = InsuranceProvider, y = HealthcareCost)) +
  geom_boxplot() +
  labs(title = "Healthcare Cost vs Insurance Provider", x = "Insurance Provider",
       y = "Healthcare Cost")
    # Create a boxplot
    # Give your boxplot a title using labs
print(ggplotly(plot_cost_by_insurance))
...
plot_visits_by_region <- ggplot(healthcare_df, aes(x = Region, y = HealthcareCost)) +
  ...
  labs(title = "Helathcare Cost vs Region", x = "Region", y = "Healthcare Cost")

# Use ggplotly() to make plot_visits_by_region interactive.
print(ggplotly(plot_visits_by_region))
